...
export default async function CityPage({ params }) {
  ...
  const indexes = await Index.find({ city: new RegExp(`^${city}$`, "i") }).lean();
  ...
  return (
    <>
      ...
      <h1 className="display hero-title">Top Businesses in {city}</h1>
      <p className="hero-sub">{indexes.length} indexes · {listings.length} listings</p>
